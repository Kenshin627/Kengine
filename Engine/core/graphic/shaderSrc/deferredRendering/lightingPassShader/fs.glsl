...
uniform sampler2D      pcssShadowMap;
...
layout (std140, binding = 3) uniform PcssLightMatriceBuffer
{
...
{
    return uLightRadiusUV * (zWorld - uLightNear) / zWorld;
}
...
{
    return uLightRadiusUV * (zReceiver - zBlocker) / zBlocker;
}

// ------------------------------------------------------------------

// Project UV size to the near plane of the light
float projectToLightUV(float sizeUv, float zWorld)
{
    return sizeUv * uLightNear / zWorld;
}
...
{
    return uLightFar * uLightNear / (uLightFar - z * (uLightFar - uLightNear));
}
                  ...
                  float     searchRegionRadiusUV)
{
    ...
    for (int i = 0; i < uBlockerSearchSamples; ++i)
    {
        ...
        if (uBlockerSearchSamples == 25)
            offset = Poisson25[i];
        else if (uBlockerSearchSamples == 32)
            offset = Poisson32[i];
        else if (uBlockerSearchSamples == 64)
            offset = Poisson64[i];
        else if (uBlockerSearchSamples == 100)
            offset = Poisson100[i];
        else if (uBlockerSearchSamples == 128)
            offset = Poisson128[i];
...
//                      random_rotation.y * offset.x + random_rotation.x * offset.y);
        ...
        float shadowMapDepth = texture(pcssShadowMap, uv + offset).r;
...
float shadowPCF(vec2 uv, float z0, float bias, float filterRadiusUv)
{
    //vec2 random_rotation = texture(s_random_angles, in_world_pos * correction_factor).rg;
    ...
    for (int i = 0; i < uPcfSamples; ++i)
    {
        ...
        if (uPcfSamples == 25)
            offset = Poisson25[i];
        else if (uPcfSamples == 32)
            offset = Poisson32[i];
        else if (uPcfSamples == 64)
            offset = Poisson64[i];
        else if (uPcfSamples == 100)
            offset = Poisson100[i];
        else if (uPcfSamples == 128)
            offset = Poisson128[i];
...
//                      random_rotation.y * offset.x + random_rotation.x * offset.y);
        ...
        float d = texture(pcssShadowMap, uv + offset).r;
         //z0 - bias
         if(d < (z0 - bias))
         {
            sum += 1.0f;
...
float shadowOcclusion(vec3 viewSpacePos)
{
    vec4 worldPos = inverse(cameraBuffer.viewMatrix) * vec4(viewSpacePos, 1.0);
	worldPos /= worldPos.w;
    vec4 lightSpacePos = pcssLightMatriceBuffer.lightViewProjectionMatrix * worldPos;
    vec3 proj_coords = lightSpacePos.xyz / lightSpacePos.w;

    vec4 lightViewSpacePos = pcssLightMatriceBuffer.lightViewMatrix * worldPos;
